...
unsigned long Mdict::reduce1(
  ...
  while (left <= right) {
    ...
    //    std::cout<<wordlist[mid]->key_word<< " " <<
    //    _s(wordlist[mid]->key_word)<<" "<<comp << " "<< left << " "<< mid  <<"
    //    " << right<<std::endl;
    if (comp > 0) {
      left = mid + 1;
    } else if (comp < 0) {
